...
kobold_authors_note = "http://tidy-clocks-exist-34-135-174-207.loca.lt/api/v1/config/authors_note"
...
def art():
    if request.method == 'POST':
        ...
        requests.put(kobold_authors_note, json={'value': session['art_style']})
        session['prompt_start'] = session['location'] + "," + session[
            'art_style'] + "," + "D&D Greg Rutkowski high-detail quality-shading"
        session['text_display'] = []
        session['text_display'].append(
            "Your " + session['art_style'] + " adventure begins in (a) " + session['location'] + "!")
...
def play_game(user_input, count):
    ...
    try:
        ...
        if user_input != "":
            ...
            try:
                kobold_ai_returned = generate_response(user_input,
                                                       "http://tidy-clocks-exist-34-135-174-207.loca.lt/api/v1/")
            except:
        ...
        if count >= 10:
            ...
            return render_template('game.html', user_image=output_url,
                                   page_text=["ERROR - Your replicate token expired"])
        count = count + 1
